...
UINT Waves::RowCount() const { return mNumRows; }

UINT Waves::ColumnCount() const { return mNumCols; }

UINT Waves::VertexCount() const { return mVertexCount; }

UINT Waves::TriangleCount() const { return mTriangleCount; }
	...
	if (t >= mTimeStep)
	{
		...
		for (UINT i = 1; i < mNumRows - 1; ++i)
		{
			for (UINT j = 1; j < mNumCols - 1; ++j)
			{
				...
				// so overwrite that buffer with the new update. Note how we can do
				// this inplace (read/write to same element) because we won't need
				// prev_ij again and the assignment happens last.

				// Note j indexes x and i indexes z: h(x_j, z_i, t_k)
				// Moreover, out +z axis goes "down"; this is just to
				// keep consistent with out row indices goind down.
		...
		// so this data needs to become the current solution and the
		// old current solution becomes the new previous solution.
