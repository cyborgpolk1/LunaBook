...
void GameTimer::Tick()
{
	...
	// Force nonnegative. The DXSDK's CDXUTTimer mentions that if the processor
...
void GameTimer::Stop()
{
	...
	if (!mStopped)
	{
		...
		// Otherwise, save the time we stopped at, and set the Boolean flag
		// indicating the timer is stopped.
		mStopTime = currTime;
		mStopped = true;
	}
}
...
void GameTimer::Start()
{
	...
	if (mStopped)
	{
		...
		// is not valid, as it occurred while paused. So reset it to current time.
		mPrevTime = startTime;
