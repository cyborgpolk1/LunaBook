...
Ssao::Ssao(ID3D11Device* device, ID3D11DeviceContext* dc, int width, int height, float fovy, float farZ)
    ...
    mSSAOVS(0), mSSAOPS(0), mBuildBuffer(0), mBlurVS(0), mBlurPS(0), mBlurBuffer(0), mNormalDepthSampler(0),
    mRandomVecSampler(0), mBlurSampler(0)
{
...
Ssao::~Ssao()
{
    ...
    ReleaseCOM(mSSAOVS);
    ReleaseCOM(mSSAOPS);
    ReleaseCOM(mBuildBuffer);
    ReleaseCOM(mBlurVS);
    ReleaseCOM(mBlurPS);
    ReleaseCOM(mBlurBuffer);

    ReleaseCOM(mNormalDepthSampler);
    ReleaseCOM(mRandomVecSampler);
    ReleaseCOM(mBlurSampler);
...
{
    // Bind the ambient mpa as the render target. Observe that this pass does not bind
    ...
    // Set shaders
    mDC->VSSetShader(mSSAOVS, 0, 0);
    mDC->PSSetShader(mSSAOPS, 0, 0);

    // Transform NDC [-1,+1]^2 to texture space [0,1]^2
    ...
    HR(mDC->Map(mBuildBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));

    BuildBuffer* dataPtr = reinterpret_cast<BuildBuffer*>(mappedResource.pData);
    dataPtr->ViewToTexSpace = XMMatrixTranspose(PT);
    dataPtr->OffsetVectors = mOffsets;
    dataPtr->FrustumCorners = mFrustumCorner;
    dataPtr->OcclusionRadius = 0.5f;
    dataPtr->OcclusionFadeStart = 0.2f;
    dataPtr->OcclusionFadeEnd = 2.0f;
    dataPtr->SurfaceEpsilon = 0.05f;

    mDC->Unmap(mBuildBuffer, 0);

    mDC->VSSetConstantBuffers(0, 1, &mBuildBuffer);
    mDC->PSSetConstantBuffers(0, 1, &mBuildBuffer);

    mDC->PSSetShaderResources(0, 1, &mNormalDepthSRV);
    mDC->PSSetShaderResources(1, 1, &mRandomVectorSRV);

    mDC->PSSetSamplers(0, 1, &mNormalDepthSampler);
    mDC->PSSetSamplers(1, 1, &mRandomVecSampler);
    ...
    mDC->PSSetShaderResources(0, 1, &nullSRV);
    mDC->PSSetShaderResources(1, 1, &nullSRV);
}
...
void Ssao::BlurAmbientMap(ID3D11ShaderResourceView* inputSRV, ID3D11RenderTargetView* outputRTV, bool horzBlur)
{
    ...
    // Set shaders
    mDC->VSSetShader(mBlurVS, 0, 0);
    mDC->PSSetShader(mBlurPS, 0, 0);

    // Set constants
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    HR(mDC->Map(mBlurBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));

    BlurBuffer* dataPtr = reinterpret_cast<BlurBuffer*>(mappedResource.pData);
    dataPtr->TexelWidth = 1.0f / mAmbientMapViewport.Width;
    dataPtr->TexelHeight = 1.0f / mAmbientMapViewport.Height;
    dataPtr->HorizontalBlur = horzBlur;

    mDC->Unmap(mBlurBuffer, 0);

    mDC->PSSetConstantBuffers(0, 1, &mBlurBuffer);

    mDC->PSSetShaderResources(0, 1, &mNormalDepthSRV);
    mDC->PSSetShaderResources(1, 1, &inputSRV);

    mDC->PSSetSamplers(0, 1, &mBlurSampler);
...
void Ssao::BuildFrustumFarCorners(float fovy, float farZ)
{
    ...
    mFrustumCorner[0] = XMFLOAT4(-halfWidth, -halfHeight, farZ, 0.0f);
    mFrustumCorner[1] = XMFLOAT4(-halfWidth, halfHeight, farZ, 0.0f);
    mFrustumCorner[2] = XMFLOAT4(halfWidth, halfHeight, farZ, 0.0f);
    mFrustumCorner[3] = XMFLOAT4(halfWidth, -halfHeight, farZ, 0.0f);
}
...
void Ssao::BuildFullScreenQuad()
{
    ...
    HR(md3dDevice->CreateBuffer(&ibd, &initData, &mScreenQuadIB));
}
...
void Ssao::BuildTextureViews()
{
    ...
    HR(md3dDevice->CreateSamplerState(&samDesc, &mNormalDepthSampler));

    samDesc.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
    samDesc.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;

    HR(md3dDevice->CreateSamplerState(&samDesc, &mBlurSampler));
}
...
void Ssao::BuildRandomVectorTexture()
{
    ...
    HR(md3dDevice->CreateSamplerState(&samDesc, &mRandomVecSampler));
}
...
void Ssao::BuildOffsetVectors()
{
    ...
    // if we choose touse less than 14 samples.
...
void Ssao::BuildFX()
{
    ...
    auto shaderDirectory = ExePath().append(L"../../../Shaders/");
    
    auto buildFile = shaderDirectory + L"BuildSSAO.hlsl";
    ShaderHelper::CreateShader(md3dDevice, &mSSAOVS, buildFile.c_str(), "VS", 0, &mInputLayout, vertexDesc, 3);
    ShaderHelper::CreateShader(md3dDevice, &mSSAOPS, buildFile.c_str(), "PS", 0);

    auto blurFile = shaderDirectory.append(L"SSAOBlur.hlsl");
    ShaderHelper::CreateShader(md3dDevice, &mBlurVS, blurFile.c_str(), "VS", 0);
    ShaderHelper::CreateShader(md3dDevice, &mBlurPS, blurFile.c_str(), "PS", 0);

    D3D11_BUFFER_DESC bufferDesc;
    bufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    bufferDesc.ByteWidth = sizeof(BuildBuffer);
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    bufferDesc.MiscFlags = 0;
    bufferDesc.StructureByteStride = 0;
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;

    HR(md3dDevice->CreateBuffer(&bufferDesc, 0, &mBuildBuffer));

    bufferDesc.ByteWidth = sizeof(BlurBuffer);

    HR(md3dDevice->CreateBuffer(&bufferDesc, 0, &mBlurBuffer));
}