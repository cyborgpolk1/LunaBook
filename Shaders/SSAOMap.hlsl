...
PixelIn BasicVS(BasicVertexIn vin)
{
    ...
    vout.SsaoPosH = mul(vout.PosH, gSSAOTransform);
...
PixelIn DS(PatchTess patchTess, float3 bary : SV_DomainLocation, const OutputPatch<HullOut, 3> tri)
{
    ...
    // Generate projective tex-coords to project SSAO map onto scene.
    dout.SsaoPosH = mul(dout.PosH, gSSAOTransform);
...
float4 PS(PixelIn pin) : SV_TARGET
{
    ...
    if (NUM_LIGHTS > 0)
    {
        ...
        float ambientAccess = gSsaoMap.Sample(gSample, pin.SsaoPosH.xy, 0.0f).r;
