...
// Determin how much the sample point q occludes the point p as a function of distZ.
float OcclusionFunction(float distZ)
{
    ...
    if (distZ > gSurfaceEpsilon)
    {
        ...
        // Linearly decrease occlusion 1 to 0 as distZ goes
...
float4 PS(VertexOut pin) : SV_TARGET
{
    ...
    float3 randVec = 2.0f * 
        gRandomVecMap.SampleLevel(samRandomVec, 4.0f * pin.Tex, 0.0f).rgb - 1.0f;
    
    float occlusionSum = 0.0f;
    
    // Sample neighboring point about p in the hemisphere oriented by n.
    [unroll]
