...
VertexOut VS(VertexIn vin)
{
    ...
    // Output vertex attributes to the next stage.
    vout.Tex = vin.Tex;
...
float CalcTessFactor(float3 p)
{
    ...
    // max norm in xz plane (useful to see detail levels from bird's eye).
...
PatchTess ConstantHS(InputPatch<VertexOut, 4> patch, uint patchID : SV_PrimitiveID)
{
    ...
    else
    {
        ...
        // tessellation factor. Otehrwise, gaps can appear.
...
DomainOut DS(PatchTess patchTess, float2 uv : SV_DomainLocation, const OutputPatch<HullOut, 4> quad)
{
    ...
    // using finit difference, but the vertices move continuously
