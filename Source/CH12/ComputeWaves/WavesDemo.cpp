...
WavesDemo::WavesDemo(HINSTANCE hInstance)
	...
	mVS(0), mPS(0), mBillVS(0), mBillGS(0), mBillPS(0), mWavesVS(0), mInputLayout(0), mTreeLayout(0),
	mPerObjectBuffer(0), mPerFrameBuffer(0), mNoCullRS(0), mGridIndexCount(0), mWavesIndexCount(0),
	mCrateTexture(0), mLandTexture(0), mWaterTexture(0), mTreeTextureArray(0), mSampleState(0), mClampSampler(0),
	mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(200.0f), mWaterTexOffset(0.0f, 0.0f),
	mEyePosW(0.0f, 0.0f, 0.0f), mTransparentBlend(0), mAlphaToCoverageBS(0), mTreeBuffer(0),mWaveBuffer(0)
{
	mMainWndCaption = L"Compute Waves Demo";
...
WavesDemo::~WavesDemo()
{
    ...
    ReleaseCOM(mTreeBuffer);
    ReleaseCOM(mWaveBuffer);
...
bool WavesDemo::Init()
{
	...
	mWaves.Init(md3dDevice, 200, 200, 0.8f, 0.03f, 3.25f, 0.4f);
...
void WavesDemo::DrawScene()
{
	...
	md3dImmediateContext->Unmap(mPerObjectBuffer, 0);
	md3dImmediateContext->VSSetConstantBuffers(1, 1, &mPerObjectBuffer);
    md3dImmediateContext->VSSetConstantBuffers(2, 1, &mWaveBuffer);
    ID3D11ShaderResourceView* displacementMap = mWaves.GetDisplacementMap();
    md3dImmediateContext->VSSetShaderResources(2, 1, &displacementMap);
    md3dImmediateContext->VSSetSamplers(1, 1, &mClampSampler);
	...
	md3dImmediateContext->VSSetShader(mWavesVS, 0, 0);
	md3dImmediateContext->PSSetShader(mPS, 0, 0);

	md3dImmediateContext->DrawIndexed(mWavesIndexCount, 0, 0);

	md3dImmediateContext->OMSetBlendState(0, blendFactors, 0xffffffff);

    ID3D11ShaderResourceView* nullSRV = nullptr;
    md3dImmediateContext->VSSetShaderResources(0, 1, &nullSRV);
    md3dImmediateContext->VSSetShaderResources(1, 1, &nullSRV);
    md3dImmediateContext->VSSetShaderResources(2, 1, &nullSRV);
...
XMFLOAT3 WavesDemo::GetHillNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void WavesDemo::BuildWavesGeometryBuffers()
{
    ...
    mWavesIndexCount = (UINT)grid.Indices.size();

	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * mWavesIndexCount;
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	D3D11_SUBRESOURCE_DATA initData;
	initData.pSysMem = grid.Indices.data();
	HR(md3dDevice->CreateBuffer(&ibd, &initData, &mWavesIB));
}

void WavesDemo::BuildCrateGeometryBuffers()
{
...
void WavesDemo::BuildTreeGeometryBuffers()
{
...
void WavesDemo::BuildFX()
{
    ...
    ShaderHelper::CreateShader(md3dDevice, &mWavesVS, ExePath().append(L"../../../Shaders/ComputeWaves.hlsl").c_str(), "WavesVS", 0);
    ...
    matrixBufferDesc.ByteWidth = sizeof(PerWave);
    matrixBufferDesc.Usage = D3D11_USAGE_IMMUTABLE;
    matrixBufferDesc.CPUAccessFlags = 0;

    PerWave inits;
    inits.DisplacementTexelSize = XMFLOAT2(1.0f / mWaves.RowCount(), 1.0f / mWaves.ColumnCount());
    inits.GridSpatialStep = mWaves.SpatialStep();

    D3D11_SUBRESOURCE_DATA waveInit;
    waveInit.pSysMem = &inits;
    HR(md3dDevice->CreateBuffer(&matrixBufferDesc, &waveInit, &mWaveBuffer));
}

void WavesDemo::BuildTex()
{
    ...
    HR(md3dDevice->CreateSamplerState(&samplerDesc, &mClampSampler));
}