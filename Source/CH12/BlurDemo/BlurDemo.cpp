...
BlurDemo::BlurDemo(HINSTANCE hInstance)
    ...
    mQuadVB(0), mQuadIB(0), mQuadIndexCount(0), mQuadVS(0), mQuadPS(0), mQuadLayout(0), mQuadBuffer(0),
    mOffScreenSRV(0), mOffScreenRTV(0), mOffScreenUAV(0)
{
...
void BlurDemo::DrawScene()
{
    ...
    md3dImmediateContext->OMSetRenderTargets(1, &mOffScreenRTV, mDepthStencilView);
    ...
    md3dImmediateContext->VSSetConstantBuffers(0, 1, &mQuadBuffer);
    md3dImmediateContext->PSSetShaderResources(0, 1, &mOffScreenSRV);
...
float BlurDemo::GetHeight(float x, float z) const
{
...
void BlurDemo::BuildTreeGeometryBuffers()
{
...
void BlurDemo::BuildQuadGeometryBuffers()
{
    ...
    vbd.ByteWidth = sizeof(UINT) * mQuadIndexCount;
    vbd.BindFlags = D3D11_BIND_INDEX_BUFFER;
    vinitData.pSysMem = quad.Indices.data();
    HR(md3dDevice->CreateBuffer(&vbd, &vinitData, &mQuadIB));
}

void BlurDemo::BuildFX()
{
    ...
    HR(md3dDevice->CreateBuffer(&constDesc, &cinitData, &mQuadBuffer));
}

void BlurDemo::BuildTex()
{
...
void BlurDemo::BuildOffScreenViews()
{
    ...
    HR(md3dDevice->CreateRenderTargetView(tex, 0, &mOffScreenRTV));
    HR(md3dDevice->CreateShaderResourceView(tex, 0, &mOffScreenSRV));
    HR(md3dDevice->CreateUnorderedAccessView(tex, 0, &mOffScreenUAV));
