...
SSAODemo::SSAODemo(HINSTANCE hInstance)
    ...
    mPerQuadBuffer(0), mQuadDS(0), mSsao(0), mNormalDepthVS(0), mNormalDepthPS(0), mNormalDepthLayout(0),
    mNormalDepthBuffer(0)
{
...
SSAODemo::~SSAODemo()
{
    ...
    ReleaseCOM(mNormalDepthLayout);
    ReleaseCOM(mNormalDepthBuffer);
}
...
void SSAODemo::DrawScene()
{
    ...
    // same dimensions as the back buffer, os we can use the screen viewport.
    ...
    ID3D11ShaderResourceView* ssaoMap = mSsao->AmbientSRV();
    ...
    dataPtr->SSAOTransform = XMMatrixTranspose(toTexSpace);
    ...
    md3dImmediateContext->PSSetShaderResources(4, 1, &ssaoMap);
...
void SSAODemo::BuildShapeGeometryBuffer()
{
...
void SSAODemo::BuildSkullGeometryBuffer()
{
	// Load skull
...
void SSAODemo::BuildFX()
{
    ...
    auto ShadowMapFilename = ExePath().append(L"../../../Shaders/SSAOMap.hlsl");
    ...
    auto NormalDepthFilename = ExePath().append(L"../../../Shaders/SSAONormalDepth.hlsl");
    auto NormalDepthString = NormalDepthFilename.c_str();

    ShaderHelper::CreateShader(md3dDevice, &mNormalDepthVS, NormalDepthString, "VS", 0, &mNormalDepthLayout, vertexDesc, 3);
    ShaderHelper::CreateShader(md3dDevice, &mNormalDepthPS, NormalDepthString, "PS", 0);
    ...
    matrixBufferDesc.ByteWidth = sizeof(NormalDepthPerObjectBuffer);

    HR(md3dDevice->CreateBuffer(&matrixBufferDesc, 0, &mNormalDepthBuffer));
}

void SSAODemo::BuildTex() 
{
...
void SSAODemo::DrawSceneToSsaoNormalDepthMap()
{
    ...
    md3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    md3dImmediateContext->IASetInputLayout(mNormalDepthLayout);
    md3dImmediateContext->IASetVertexBuffers(0, 1, &mShapesVB, &stride, &offset);
    md3dImmediateContext->IASetIndexBuffer(mShapesIB, DXGI_FORMAT_R32_UINT, 0);

    md3dImmediateContext->VSSetShader(mNormalDepthVS, 0, 0);
    md3dImmediateContext->PSSetShader(mNormalDepthPS, 0, 0);

    D3D11_MAPPED_SUBRESOURCE mappedResource;

    // Draw the grid
    XMMATRIX world = XMLoadFloat4x4(&mGridWorld);
    // Note: We actually send W^T to the shader. ((W^T)^-1)^T = ((W^-1)^T)^T = (W^-1).
    XMMATRIX worldInvTranspose = XMMatrixInverse(&XMMatrixDeterminant(world), world);
    HR(md3dImmediateContext->Map(mNormalDepthBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));
    NormalDepthPerObjectBuffer* dataPtr = (NormalDepthPerObjectBuffer*)mappedResource.pData;
    dataPtr->WorldView = XMMatrixTranspose(world * view);
    dataPtr->WorldInvTransposeView = XMMatrixTranspose(worldInvTranspose * view);
    dataPtr->WorldViewProj = XMMatrixTranspose(world * view * proj);
    dataPtr->gTexTransform = XMMatrixTranspose(XMMatrixScaling(8.0f, 10.0f, 1.0f));
    md3dImmediateContext->Unmap(mNormalDepthBuffer, 0);
    md3dImmediateContext->VSSetConstantBuffers(0, 1, &mNormalDepthBuffer);
    md3dImmediateContext->PSSetShaderResources(0, 1, &mFloorTex);
    md3dImmediateContext->PSSetSamplers(0, 1, &mSampleState);
    md3dImmediateContext->DrawIndexed(mGridIndexCount, mGridIndexOffset, mGridVertexOffset);
    ...
    HR(md3dImmediateContext->Map(mNormalDepthBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));
    dataPtr = (NormalDepthPerObjectBuffer*)mappedResource.pData;
    dataPtr->WorldView = XMMatrixTranspose(world * view);
    dataPtr->WorldInvTransposeView = XMMatrixTranspose(worldInvTranspose * view);
    dataPtr->WorldViewProj = XMMatrixTranspose(world * view * proj);
    dataPtr->gTexTransform = XMMatrixTranspose(XMMatrixScaling(2.0f, 1.0f, 1.0f));
...
void SSAODemo::BuildScreenQuadGeometryBuffer()
{
...
void SSAODemo::DrawScreenQuad()
{
    ...
    md3dImmediateContext->PSSetSamplers(0, 1, &mSampleState);
    ID3D11ShaderResourceView* srv = mSsao->AmbientSRV();
