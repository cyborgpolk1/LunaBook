...
void LitWavesDemo::UpdateScene(float dt)
{
...
void LitWavesDemo::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void LitWavesDemo::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void LitWavesDemo::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float LitWavesDemo::GetHeight(float x, float z) const
{
...
void LitWavesDemo::BuildWavesGeometryBuffers()
{
...
void LitWavesDemo::BuildFX()
{
	...
	ShaderHelper::CreateShader(md3dDevice, &mVS, ExePath().append(L"../../../Shaders/BasicLighting.hlsl").c_str(), "VS", 0, &mInputLayout, vertexDesc, 2);
	ShaderHelper::CreateShader(md3dDevice, &mPS, ExePath().append(L"../../../Shaders/BasicLighting.hlsl").c_str(), "PS", 0);
