...
D3DMAIN(ToonWavesDemo);

ToonWavesDemo::ToonWavesDemo(HINSTANCE hInstance)
...
{
	mMainWndCaption = L"Toon Waves Demo";
...
void ToonWavesDemo::UpdateScene(float dt)
{
...
void ToonWavesDemo::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void ToonWavesDemo::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void ToonWavesDemo::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float ToonWavesDemo::GetHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 ToonWavesDemo::GetHillNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void ToonWavesDemo::BuildLandGeometryBuffers()
{
...
void ToonWavesDemo::BuildWavesGeometryBuffers()
{
...
void ToonWavesDemo::BuildFX()
{
	...
	ShaderHelper::CreateShader(md3dDevice, &mVS, ExePath().append(L"../../../Shaders/ToonLighting.hlsl").c_str(), "VS", 0, &mInputLayout, vertexDesc, 2);
	ShaderHelper::CreateShader(md3dDevice, &mPS, ExePath().append(L"../../../Shaders/ToonLighting.hlsl").c_str(), "PS", 0);
