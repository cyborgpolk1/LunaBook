#include <windows.h>
#include <DirectXMath.h>
#include <iostream>

using namespace std;
using namespace DirectX;

// Overload the "<<" operators so that we can use the cout to
// output XMVECTOR objects
ostream& operator<<(ostream& os, FXMVECTOR v)
{
	XMFLOAT3 dest;
	XMStoreFloat3(&dest, v);

	os << "(" << dest.x << ", " << dest.y << ", " << dest.z << ")";
	return os;
}

int main()
{
	cout.setf(ios_base::boolalpha);

	// Check support for SSE2 (Pentium4, AMD K8, and above)
	if (!XMVerifyCPUSupport())
	{
		cout << "xna math not supported" << endl;
		return 0;
	}

	XMVECTOR q = XMVectorZero();
	XMVECTOR w = XMVectorSplatOne();
	XMVECTOR e = XMVectorSet(1.0f, 2.0f, 3.0f, 4.0f);
	XMVECTOR r = XMVectorReplicate(-2.0f);
	XMVECTOR t = XMVectorSplatZ(e);

	cout << "q = " << q << endl;
	cout << "w = " << w << endl;
	cout << "e = " << e << endl;
	cout << "r = " << r << endl;
	cout << "t = " << t << endl;


	XMVECTOR n = XMVectorSet(1.0f, 0.0f, 0.0f, 0.0f);
	XMVECTOR u = XMVectorSet(1.0f, 2.0f, 3.0f, 0.0f);
	XMVECTOR v = XMVectorSet(-2.0f, 1.0f, -3.0f, 0.0f);
	w = XMVectorSet(0.707f, 0.707f, 0.0f, 0.0f);

	// Vector addition: XMVECTOR operator +
	XMVECTOR a = u + v;
	// Vector subtraction: XMVECTOR operator -
	XMVECTOR b = u - v;

	// Scalar multiplcation: XMVECTOR operator *
	XMVECTOR c = 10.0f * u;

	// ||u||
	XMVECTOR L = XMVector3Length(u);

	// d = u / ||u||
	XMVECTOR d = XMVector3Normalize(u);

	// s = u dot v
	XMVECTOR s = XMVector3Dot(u, v);

	// x = u x v
	XMVECTOR x = XMVector3Cross(u, v);

	// Find proj_n(w) and perp_n(w)
	XMVECTOR projW, perpW;
	XMVector3ComponentsFromNormal(&projW, &perpW, w, n);

	// Does projW + perpW == w?
	bool equal = XMVector3Equal(projW + perpW, w);
	bool notEqual = XMVector3NotEqual(projW + perpW, w);

	// The angle between projW and perpW should be 90 degrees
	XMVECTOR angleVec = XMVector3AngleBetweenVectors(projW, perpW);
	float angleRadians = XMVectorGetX(angleVec);
	float angleDegrees = XMConvertToDegrees(angleRadians);


	cout << "u                                  = " << u << endl;
	cout << "v                                  = " << v << endl;
	cout << "w                                  = " << w << endl;
	cout << "n                                  = " << n << endl;
	cout << "a = u + v                          = " << a << endl;
	cout << "b = u - v                          = " << b << endl;
	cout << "c = 10 * u                         = " << c << endl;
	cout << "d = u / ||u||                      = " << d << endl;
	cout << "x = u x v                          = " << x << endl;
	cout << "L = ||u||                          = " << L << endl;
	cout << "s = u.v                            = " << s << endl;
	cout << "projW                              = " << projW << endl;
	cout << "perpW                              = " << perpW << endl;
	cout << "projW + perpW == w                 = " << equal << endl;
	cout << "projW + perpW != w                 = " << notEqual << endl;
	cout << "angle                              = " << angleDegrees << endl;

	cout << endl;
	return 0;
}