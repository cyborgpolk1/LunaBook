...
D3DMAIN(OverdrawBlendApp);

OverdrawBlendApp::OverdrawBlendApp(HINSTANCE hInstance)
	...
	mEyePosW(0.0f, 0.0f, 0.0f), mCrateVB(0), mCrateIB(0), mCrateTexture(0), mOverdrawBS(0)
{
	mMainWndCaption = L"Overdraw (Blend) Demo";
...
OverdrawBlendApp::~OverdrawBlendApp()
{
	ReleaseCOM(mLandVB);
	ReleaseCOM(mLandIB);
	ReleaseCOM(mWavesVB);
	...
	ReleaseCOM(mOverdrawBS);
}

bool OverdrawBlendApp::Init()
{
	if (!D3DApp::Init())
		return false;
	...
	BuildFX();
	BuildTex();
	BuildRaserizerStates();
	BuildBlendStates();

	return true;
}

void OverdrawBlendApp::OnResize()
{
	D3DApp::OnResize();
...
void OverdrawBlendApp::UpdateScene(float dt)
{
...
void OverdrawBlendApp::DrawScene()
{
	...
	md3dImmediateContext->OMSetBlendState(mOverdrawBS, blendFactors, 0xffffffff);
...
void OverdrawBlendApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void OverdrawBlendApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void OverdrawBlendApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float OverdrawBlendApp::GetHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 OverdrawBlendApp::GetHillNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void OverdrawBlendApp::BuildLandGeometryBuffers()
{
...
void OverdrawBlendApp::BuildWavesGeometryBuffers()
{
...
void OverdrawBlendApp::BuildCrateGeometryBuffers()
{
...
void OverdrawBlendApp::BuildTex()
{
...
void OverdrawBlendApp::BuildRaserizerStates()
{
...
void OverdrawBlendApp::BuildBlendStates()
{
	...
	HR(md3dDevice->CreateBlendState(&blendDesc, &mOverdrawBS));
}