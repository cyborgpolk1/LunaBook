...
CrateApp::~CrateApp()
{
	...
	ReleaseCOM(mFlareTexture);
	ReleaseCOM(mMaskTexture);
...
void CrateApp::DrawScene()
{
	...
	md3dImmediateContext->PSSetShaderResources(0, 1, &mFlareTexture);
	md3dImmediateContext->PSSetShaderResources(1, 1, &mMaskTexture);
...
void CrateApp::BuildFX()
{
	...
	ShaderHelper::CreateShader(md3dDevice, &mVS, ExePath().append(L"../../../Shaders/Multitexture.hlsl").c_str(), "VS", 0, &mInputLayout, vertexDesc, 2);
	ShaderHelper::CreateShader(md3dDevice, &mPS, ExePath().append(L"../../../Shaders/Multitexture.hlsl").c_str(), "PS", 0);
