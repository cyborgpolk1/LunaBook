#include "FilterCrates.h"
#include <d3dcompiler.h>
#include <vector>

D3DMAIN(FilterCratesApp);

FilterCratesApp::FilterCratesApp(HINSTANCE hInstance)
...
FilterCratesApp::~FilterCratesApp()
{
	ReleaseCOM(mVB);
	ReleaseCOM(mIB);
...
void FilterCratesApp::OnResize()
{
	D3DApp::OnResize();

	// The window resized, so update the aspect ratio and recompile the projection matrix.
	XMMATRIX P = XMMatrixPerspectiveFovLH(0.25f * MathHelper::Pi, AspectRatio() / 3.0f, 1.0f, 1000.0f);
	XMStoreFloat4x4(&mProj, P);

	BuildViewports();
}

void FilterCratesApp::UpdateScene(float dt)
{
...
void FilterCratesApp::DrawScene()
{
...
void FilterCratesApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePoint.x = x;
	mLastMousePoint.y = y;
...
void FilterCratesApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void FilterCratesApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void FilterCratesApp::BuildGeometryBuffers()
{
...
void FilterCratesApp::BuildViewports()
{
	...
	a.TopLeftX = 0.0f;
	a.TopLeftY = 0.0f;
	a.Width = static_cast<float>(mClientWidth) / 3.0f;
	a.Height = static_cast<float>(mClientHeight);
	a.MinDepth = 0.0f;
	a.MaxDepth = 1.0f;
