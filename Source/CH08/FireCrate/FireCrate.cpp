...
D3DMAIN(FireCrateApp);

FireCrateApp::FireCrateApp(HINSTANCE hInstance)
	: D3DApp(hInstance), mVB(0), mIB(0), mInputLayout(0), mVS(0), mPS(0), mMatrixBuffer(0),
	mTheta(1.5f*MathHelper::Pi), mPhi(0.25f*MathHelper::Pi), mRadius(5.0f), currentFrame(0)
{
	mMainWndCaption = L"Fire mCrate Demo";
...
void FireCrateApp::UpdateScene(float dt)
{
	...
	if ((mTimer.TotalTime() - t_base) >= 1.0f / 30.0f) 
	{
		t_base += 1.0f / 30.0f;
...
void FireCrateApp::DrawScene()
{
	...
	md3dImmediateContext->PSSetShaderResources(0, 1, &mTexture[currentFrame]);
...
void FireCrateApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePoint.x = x;
	mLastMousePoint.y = y;
...
void FireCrateApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void FireCrateApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void FireCrateApp::BuildGeometryBuffers()
{
...
void FireCrateApp::BuildTex()
{
	...
	for (int i = 0; i < 120; i++)
	{
		...
		HR(CreateWICTextureFromFile(md3dDevice, ss.str().c_str(), &textureResource[i], &mTexture[i]));
	}
