...
void QuadBezier::DrawScene()
{
    md3dImmediateContext->ClearRenderTargetView(mRenderTargetView, reinterpret_cast<const float*>(&Colors::Blue));
    md3dImmediateContext->ClearDepthStencilView(mDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);

    md3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_9_CONTROL_POINT_PATCHLIST);
...
void QuadBezier::OnMouseDown(WPARAM btnState, int x, int y)
{
    mLastMousePos.x = x;
    mLastMousePos.y = y;
...
void QuadBezier::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void QuadBezier::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void QuadBezier::BuildQuadPatchBuffer()
{
...
void QuadBezier::BuildFX()
{
    ...
    ShaderHelper::CreateShader(md3dDevice, &mHS, cstr, "QuadHS", 0);
    ShaderHelper::CreateShader(md3dDevice, &mDS, cstr, "QuadDS", 0);
